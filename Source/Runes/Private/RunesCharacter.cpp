...
void ARunesCharacter::BeginPlay()
{
	...
	if (SpellSlotsWidgetClass) 
	{
		SpellSlotsWidget = CreateWidget<USpellSlotsWidget>(GetWorld(), SpellSlotsWidgetClass);
		if (SpellSlotsWidget)
		{
			SpellSlotsWidget->AddToViewport();
		}
	}
...
void ARunesCharacter::Invoke()
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Invoked2"));
	if (SpellSlotsWidget)
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Invoked4"));
		SpellSlotsWidget->UpdateSpellSlots(RuneInvoker->GetInvokedSpells());
	}
	GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Invoked3"));
}
...
void ARunesCharacter::CastSpell1()
{
	...
	if (RuneInvoker->GetInvokedSpells().Num() >= 1)
	{
		...
		// Later: pull from lock-on system
		Context.Target = nullptr;

		// Direction: lock-on OR stick
		FVector InputDir = GetLastMovementInputVector();
		Context.CastDirection = InputDir.IsNearlyZero()
		...
		RuneInvoker->GetInvokedSpells()[0]->Cast(Context);
	}
}
...
void ARunesCharacter::CastSpell2()
{
	...
	if (RuneInvoker->GetInvokedSpells().Num() >= 2)
	{
		...
		RuneInvoker->GetInvokedSpells()[1]->Cast(Context);
	}
}
...
void ARunesCharacter::CastSpell3()
{
	...
	if (RuneInvoker->GetInvokedSpells().Num() >= 3)
	{
		...
		RuneInvoker->GetInvokedSpells()[2]->Cast(Context);
	}
}
